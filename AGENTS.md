...
- `<2026-09-26: docs/design.md is the default-tenant design system (Vereda Expeditions). The alert tokens warning/timeout now live in THEME_TOKEN_KEYS, the Tailwind preset, styles.css and Badge; the escalation AMBER state uses the timeout variant (not secondary). The default tenant is renamed to vereda-expeditions / "Vereda Expeditions" with primary #2C5F52.>`
- `<2026-09-26: Bricolage Grotesque is self-hosted at tenant/assets/fonts/BricolageGrotesque.woff2 (variable, latin subset) and loaded via next/font/local in apps/{storefront,backoffice}/src/lib/font.ts, setting --ota-font-sans; this is a build-time asset path, not a core import of tenant config. Design radii are fixed tokens --ota-radius-sm/md/lg (4/10/20px); the tenant theme.borderRadius still selects the legacy --ota-radius used by bare `rounded`; elevation is the two warm-tinted shadow-ota-1/ota-2 levels and Card is flat.>`
- `<2026-09-26: the back-office uses next-intl WITHOUT URL prefixes: locale resolves cookie OTA_LOCALE -> Accept-Language -> tenant primary locale; the plugin path is createNextIntlPlugin('./src/i18n/request.ts'). Keep LOCALE_COOKIE in apps/backoffice/src/i18n/locale.ts (client-safe); importing it from request.ts pulls next/headers into a client bundle. English output must stay byte-identical or the e2e back-office specs (which run with Playwright's en-US locale) break. Migrated so far: shell/nav/login, every ops page, and the reservation/inventory action forms; the inventory edit/media, pricing-rules, availability, supplier-credential, import-wizard and package create/edit form components still hold hardcoded copy.>`
