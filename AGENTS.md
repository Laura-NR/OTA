...
- `<2026-09-25: docs/pii-at-rest-review.md inventories every PII location and recommends a tiered retention-purge scope. The current purge scrubs only User PII (email/fullName/phone/image/nationality) and revokes sessions/accounts/passkeys; free-text PII (messages/reviews/incidents/customItineraryPayload.notes/decline reasons/audit metadata), generated PDFs, Better Auth Verification rows, and supplier applications are NOT scrubbed — the scope is a pending owner decision (G1–G4 in that doc).>`
