...
- `<2026-09-25: TropiPay is the chosen card rail but is NOT implemented yet (no credentials / unverified API). The community tropipayjs SDK documents clientId/clientSecret, create payment card -> shortUrl, urlNotification webhooks, and dev vs production environments. Do not add a TropiPay SDK; implement it behind PaymentProvider with plain fetch once the contract and webhook verification are confirmed, and only then add a public webhook route (ADR 0003).>`
