...
- `<2026-09-26: docs/design.md is the default-tenant design system (Vereda Expeditions). The alert tokens warning/timeout now live in THEME_TOKEN_KEYS, the Tailwind preset, styles.css and Badge; the escalation AMBER state uses the timeout variant (not secondary). The demo tenant uses the vereda palette with primary #2C5F52. Open design work — Bricolage typography, the design radii/elevation scheme, back-office i18n and a dark /ops surface — is listed under "Design gap" in docs/development-plan.md.>`
